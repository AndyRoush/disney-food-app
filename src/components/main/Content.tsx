...
import { Search, Filter, Heart, Check, Star, X, MapPin, Calendar } from 'lucide-react';
...
const DisneyFoodApp: React.FC = () => {
  ...
  const [favorites, setFavorites] = useState<number[]>(() => getStoredData('disney-food-favorites', []));
  const [eaten, setEaten] = useState<number[]>(() => getStoredData('disney-food-eaten', []));
  const [ratings, setRatings] = useState<UserRatings>(() => getStoredData('disney-food-ratings', {}));
  ...
  const parks = useMemo(() => [...new Set(foodData.map(item => item.park))], []);
  const types = useMemo(() => [...new Set(foodData.map(item => item.type))], []);
  const zones = useMemo(() => [...new Set(foodData.map(item => item.zone))], []);
  const locations = useMemo(() => [...new Set(foodData.map(item => item.location))].sort(), []);
  ...
  const filteredData = useMemo(() => {
    return foodData.filter(item => {
      ...
      const matchesAlcoholic = alcoholicFilter === 'all' || 
                              (alcoholicFilter === 'yes' && item.alcoholic) ||
                              (alcoholicFilter === 'no' && !item.alcoholic);
      const matchesPlantBased = plantBasedFilter === 'all' || 
      ...
      return matchesSearch && matchesPark && matchesType && matchesZone && matchesLocation && 
             matchesAlcoholic && matchesPlantBased && matchesFavorites && matchesNew;
    });
  }, [searchTerm, selectedPark, selectedType, selectedZone, selectedLocation, alcoholicFilter, 
      plantBasedFilter, showFavoritesOnly, showNewOnly, favorites]);
  ...
  const StarRating: React.FC<StarRatingProps> = ({ itemId, rating, onRate, size = 'w-5 h-5' }) => {
    return (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex gap-1 flex-shrink-0">
              <button
                ...
                className={`p-1 ${favorites.includes(item.id) ? 'text-red-500' : 'text-gray-400'} hover:text-red-500 transition-colors`}
              >
              ...
              <button
                ...
                className={`p-1 ${eaten.includes(item.id) ? 'text-green-500' : 'text-gray-400'} hover:text-green-500 transition-colors`}
              >
                <Check className={`w-4 h-4 ${eaten.includes(item.id) ? 'fill-current' : ''}`} />
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex gap-2">
              ...
              {item.isPlantBased && (
                <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">Plant-based</span>
              )}
              {item.alcoholic && (
                <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded-full">Alcoholic</span>
              )}
            </div>
            ...
            {ratings[item.id] && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-600">{ratings[item.id]}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-gray-600">Available: {item.dateFrom} - {item.dateTo}</span>
            </div>
            ...
            <div className="flex gap-2">
              ...
              {item.isPlantBased && (
                <span className="px-3 py-1 bg-green-100 text-green-800 text-sm rounded-full">Plant-based</span>
              )}
              {item.alcoholic && (
                <span className="px-3 py-1 bg-purple-100 text-purple-800 text-sm rounded-full">Alcoholic</span>
              )}
            </div>
            
            <div className="border-t pt-4">
              <div className="flex items-center justify-between mb-3">
                <span className="font-medium text-gray-900">Rate this item:</span>
                <StarRating 
                  itemId={item.id} 
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 bg-white shadow-sm z-40">
        <div className="px-4 py-3">
          <h1 className="text-xl font-bold text-gray-900 text-center">Disney Food Guide</h1>
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                showFavoritesOnly ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-700'
              }`}
            >
              Favorites Only
            </button>
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                showNewOnly ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'
              }`}
            >
              New Only
            </button>
